...
let solutionFile  = "src/FsShelter.Extras.sln"
...
let testAssemblies = "src/FsShelter.Extras.Tests/bin/**/*.Tests.dll"
...
Target "Package" (fun _ ->
    runDotnet 
        "src/FsShelter.Extras"
        (assemblyInfo |> Seq.fold (fun s (p,v) -> s + (sprintf " /p:%s=\"%s\"" p v)) "pack -c Release --no-build")
)

Target "PublishNuget" (fun _ ->
     runDotnet "src/FsShelter.Extras" (sprintf "nuget push bin/Release/FsShelter.Extras.%s.nupkg -s nuget.org -k %s" release.NugetVersion (environVar "nugetkey"))
)
